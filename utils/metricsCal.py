...
def evaluate_ignore(model, dataloader, device, is_train=True, threshold=0.5):
    model.eval()
    y_true = torch.tensor([],dtype=torch.int)
    y_score = torch.tensor([])
    #for data in tqdm(dataloader):
    ...
    for data in dataloader:
        ...
        if 1==1:
            inputs,y = data
            inputs = inputs.to(device)
            out,_ = model(inputs)   #这里忽略模型第二个输出
        out = out.squeeze(dim=-1)
        #out = torch.sigmoid(out)
        y_true = torch.cat((y_true, y.int().detach().cpu()))
        y_score = torch.cat((y_score, out.detach().cpu()))  #detach去除梯度，然后cpu()，然后cat将其连接起来
...
def evaluate_Bert6mA_fine_tuning(model, dataloader, device, is_train=True, threshold=0.5):
    model.eval()
    y_true = torch.tensor([],dtype=torch.int)
    y_score = torch.tensor([])
    fine_all = torch.tensor(np.empty (shape= [0,38,100]))
    #for data in tqdm(dataloader):
    model = model.to(device)
    for data in dataloader:
        #if not isinstance(model, Res_Net):
        ...
        fine_all = torch.cat((fine_all, fine.detach().cpu()))
    return fine_all
    
def evaluate_MultiLabel_fine_tuning(model, dataloader, device, is_train=True, threshold=0.5):
    model.eval()
    y_true = torch.tensor([],dtype=torch.int)
    y_score = torch.tensor([])
    fine_all = torch.tensor(np.empty (shape= [0,39,300]))
    #for data in tqdm(dataloader):
    model = model.to(device)
    for data in dataloader:
        #if not isinstance(model, Res_Net):
...
def evaluate_Embedding_fine_tuning(model, dataloader, device, is_train=True, threshold=0.5):
    model.eval()
    y_true = torch.tensor([],dtype=torch.int)
    y_score = torch.tensor([])
    fine_all = torch.tensor(np.empty (shape= [0,39,100]))
    #for data in tqdm(dataloader):
    model = model.to(device)
    for data in dataloader:
        #if not isinstance(model, Res_Net):
...
def get_train_metrics(y_pred, y_true):  # 获取训练指标
    ...
    AUC = roc_auc_score(y_true, y_pred.reshape(-1))
